...
module.exports = (adapter) => {
  ...
  const sanitizeConfig = () => {
    ...
    if (!changed) {
      for (let index = 0; index < dataPoints.length; index += 1) {
        const originalId = normalizeObjectId(original[index]?.objectId);
        if (originalId !== dataPoints[index].objectId) {
          changed = true;
          break;
        }
      }
  ...
  function normalize() {
    ...
    const dataPoints = sanitizeDataPoints(native.dataPoints);
