...
module.exports = () => ({
    async syncConfigToStates() {
    // Mirror selected native config into read-only datapoints (debugging / visibility)
    try {
      await this.setStateAsync('config.mode', String(this.config.mode ?? ''), true);
      await this.setStateAsync('config.dryRun', !!this.config.dryRun, true);
      await this.setStateAsync('config.intervalMin', Number(this.config.intervalMin ?? 0), true);

      const tgEnabled = !!(this.config.telegram && this.config.telegram.enabled);
      const tgChatId = this.config.telegram && this.config.telegram.chatId ? String(this.config.telegram.chatId) : '';
      await this.setStateAsync('config.telegram.enabled', tgEnabled, true);
      await this.setStateAsync('config.telegram.chatId', tgChatId, true);

      // Prefer nested dailyReport object (new), fallback to legacy flat keys
      const dailyCfg = this.config.dailyReport || {};
      const enabled = dailyCfg.enabled ?? this.config.dailyReportEnabled ?? false;
      const time = dailyCfg.time ?? this.config.dailyReportTime ?? '';
      const days = dailyCfg.days ?? this.config.dailyReportDays ?? [];

      await this.setStateAsync('config.dailyReport.enabled', !!enabled, true);
      await this.setStateAsync('config.dailyReport.time', String(time || ''), true);
      await this.setStateAsync('config.dailyReport.days', JSON.stringify(Array.isArray(days) ? days : []), true);
    } catch (e) {
      // soft fail - do not break adapter startup
      if (this.config && this.config.debug) {
        this.log.info(`[DEBUG] syncConfigToStates failed: ${e.message}`);
      }
    }
  },
